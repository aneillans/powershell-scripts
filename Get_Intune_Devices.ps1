...
Write-Host
$department = Read-Host -Prompt "Please specify the department you wish to filter on - leave blank for all"
$usersInDepartment = @()
if ($department)
{
...
{
    Write-Host "Loading all users devices"
}
Write-Host
...
if($Devices){
    ...
    if ($department)
    {
        if ($usersInDepartment.Contains($DeviceNoHardware.userPrincipalName) -eq $false)
        {
            continue;
        }
    }
